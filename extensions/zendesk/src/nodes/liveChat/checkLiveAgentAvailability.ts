...
export const checkLiveAgentAvailabilityNode = createNodeDescriptor({
  ...
  dependencies: {
    children: ["onAgentAvailable", "onNoAgentAvailable"],
  },
  function: async ({
    cognigy,
    config,
    childConfigs,
  }: IGetAccountStatusParams) => {
    const { api } = cognigy;
    const {
      connection,
      storeLocation,
    ...
    try {
      ...
      if (response.data?.content?.data?.agents_online === 0) {
        const onOfflineChild = childConfigs.find(
          (child) => child.type === "onNoAgentAvailable",
        );
        ...
        const onAvailableChild = childConfigs.find(
          (child) => child.type === "onAgentAvailable",
        );
